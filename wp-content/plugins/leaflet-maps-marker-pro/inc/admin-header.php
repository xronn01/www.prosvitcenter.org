...
if ( ($action == 'add') || ($action == 'edit') || ($action == 'duplicate') ) {
	...
	if (isset($layer_json_check)) { //info: check if marker exists
		if (is_numeric($layer_json_check)) { //info: check if not JSON encoded
			...
			echo '<p><div id="database-upgrade" class="error" style="padding:10px;margin:10px 0;">' . sprintf(__('Warning: database tables for Maps Marker Pro need to be updated! This is usually needed when you switched back and forth between "Leaflet Maps Marker" and "Maps Marker Pro" multiple times.<br/><a href="%1s">Please click here to start the database upgrade</a>','lmm'), LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_tools&action=database_upgrade&_wpnonce=' . $toolnonce) . '</div></p>';
		}
	}
	?>
