				...
				<?php
					foreach ($layerlist as $row) {
						...
						if ($row['multi_layer_map'] == 0) {
							echo '<option value="' . $row['id'] . '"' . ( ((@in_array($row['id'], $layer)) || ($row['id'] == $addtoLayer)) ? ' selected="selected"' : '') . ' title="' . stripslashes(htmlspecialchars($row['name'])) . '">' . mb_substr(stripslashes(htmlspecialchars($row['name'])), 0, 28) . $layername_abstract . ' (ID ' . $row['id'] . ')</option>';
						} else {
