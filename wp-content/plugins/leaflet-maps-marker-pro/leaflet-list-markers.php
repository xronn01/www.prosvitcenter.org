...
if ($action == 'search') {
	...
	$marker_per_page = intval($lmm_options[ 'markers_per_page' ]);
 	...
 	$marklist = $wpdb->get_results( "SELECT m.id,m.basemap,m.icon,m.popuptext,m.layer,m.zoom,m.openpopup as openpopup,m.lat,m.lon,m.mapwidth,m.mapheight,m.mapwidthunit,m.markername,m.panel,m.createdby,m.createdon,m.updatedby,m.updatedon,m.controlbox,m.overlays_custom,m.overlays_custom2,m.overlays_custom3,m.overlays_custom4,m.wms,m.wms2,m.wms3,m.wms4,m.wms5,m.wms6,m.wms7,m.wms8,m.wms9,m.wms10,m.address FROM `$table_name_markers` AS m order by $columnsort $columnsortorder LIMIT $marker_per_page OFFSET $offset", ARRAY_A);
}
//info:  get pagination
...
<div class="wrap">
	...
	if ( ($deleteselected == '1') && ($checkedmarkers != NULL) && current_user_can( $lmm_options[ 'capabilities_delete_others' ]) ) {
		...
		echo '<p><div class="updated" style="padding:10px;">' . __('The selected markers have been assigned to the selected layer','lmm') . ' (' . __('Marker','lmm') . ' ID ' . htmlspecialchars($checked_markers) . ', ' . __('Layer','lmm') . ' ID ' . htmlspecialchars(is_array($_POST['layer'])?implode(',', $_POST['layer']):$_POST['layer']) . ')</div>';
  ...
  if (count($marklist) < 1) {
    ...
    foreach ($marklist as $row) {
		...
		**/		
		$rowlayername = '';
		$marker_layers = json_decode($row['layer']);
	...
	?>
		...
		<?php if ( (current_user_can( $lmm_options[ 'capabilities_delete_others' ])) || (current_user_can( $lmm_options[ 'capabilities_edit_others' ])) ) { ?>
			...
			<?php 
			if (current_user_can( $lmm_options[ 'capabilities_edit_others' ])) { ?>
				<input type="radio" id="assignselected" name="bulkactions-markers" value="assignselected" /> <label for="assignselected"><?php _e('assign to the following layer(s):','lmm') ?></label>
